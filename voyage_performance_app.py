...
st.caption("Compare voyages and quantify performance deltas by any t/1000nm metric.")

# Data loader: prefer uploaded file; else voyage_data.csv next to the script; else fallback to embedded TSV.
uploaded = st.sidebar.file_uploader("Upload data (CSV/TSV with the same columns)", type=["csv","tsv"])

embedded_tsv = """Voy no	Pre-DD / Post-DD	B/L	Combined	Avg Draft	Cargo Qty	EOSP Date	Dur (COSP-EOSP) (Hrs)	NM	ME FO	ME GO	AE FO	AE GO	ME+AE	Blr	t/1000nm(ME-FO)	t/1000nm(ME-GO)	t/1000nm(AE-FO)	t/1000nm(AE-GO)	t/1000nm(Total ME)	t/1000nm(Total AE)	t/1000nm(Total ME+AE)	t/1000nm(Blr)	From	To
...
def load_data():
    ...
    # try local file
    for path in ["voyage_data.csv", "voyage_data.tsv"]:
    ...
    # fallback
    return pd.read_csv(StringIO(embedded_tsv), sep="\t")

df = load_data()

# Ensure numeric cols
for col in df.columns:
    if col not in ["Voy no", "Pre-DD / Post-DD", "B/L", "Combined", "From", "To"]:
        df[col] = pd.to_numeric(df[col], errors="coerce")

metric_cols = [c for c in df.columns if c.startswith("t/1000nm")]
default_metric = "t/1000nm(Total ME+AE)" if "t/1000nm(Total ME+AE)" in metric_cols else metric_cols[0]
...
metric = st.sidebar.selectbox("Metric (t/1000nm)", metric_cols, index=metric_cols.index(default_metric))

# Voyage multiselect (after applying phase/mode filters)
filtered = df.copy()
if phase != "All":
...
# Main: group-by table and bar chart
left, right = st.columns([2,1])

with left:
    st.subheader("Selected Voyages — Values")
    show_cols = ["Voy no","Pre-DD / Post-DD","Combined",metric]
    st.dataframe(view[show_cols].sort_values(["Combined","Pre-DD / Post-DD","Voy no"]).reset_index(drop=True))

    st.subheader("Comparison Chart")
    fig, ax = plt.subplots()
    x = np.arange(len(view))
    ax.bar(x, view[metric].values)
    ax.set_xticks(x)
    ax.set_xticklabels(view["Voy no"].astype(str).tolist(), rotation=45, ha="right")
    ax.set_ylabel(metric)
    ax.set_xlabel("Voy no")
    ax.set_title(f"Voyage Comparison: {metric}")
    st.pyplot(fig)

with right:
    st.subheader("Phase Averages (by Combined)")
    g = (df[df["Voy no"].astype(str).isin(pick_voys) if pick_voys else [True]]
    ...
    for m in [metric]:
        for comb, chunk in df.groupby("Combined"):
            pre = chunk.loc[chunk["Pre-DD / Post-DD"]=="Pre-DD", m].mean()
            post = chunk.loc[chunk["Pre-DD / Post-DD"]=="Post-DD", m].mean()
            if pd.notna(pre) and pd.notna(post):
                ...
                delta = np.nan
                pct = np.nan
            rows.append({"Combined": comb, "Metric": m, "Pre": pre, "Post": post, "Δ (Post-Pre)": delta, "% change": pct})
